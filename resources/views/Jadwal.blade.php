...
<body class="bg-gray-100 h-screen flex font-sans">
    ...
    <main class="flex-grow bg-gray-100 p-8">
     ...
     <input type="text" id="searchInput" placeholder="Cari..." class="border p-2 rounded w-1/3">
                ...
                <div class="shadow overflow-x-auto border-b border-gray-200 rounded">
                    <table class="min-w-full bg-white table-auto border border-gray-400 border-collapse">
                        <thead class="bg-gray-300">
                            ...
                            <th class="border p-2">Nama Ruangan</th>
                                <th class="px-4 py-2">Waktu Sidang</th>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="px-4 py-2">{{ $jadwal['nama_mahasiswa']}}</td>
                                <td class="px-4 py-2">{{ $jadwal['nama_ruangan']}}</td>
                                <td class="px-4 py-2">{{ $jadwal['waktu_sidang']}}</td>
                                <td class="px-4 py-2">
   ...
   Edit
                                    ...
                                    <!-- <a href="#" class="text-red-600 hover:text-red-900">Hapus</a> -->
                                     ...
                                     <!-- Form hapus -->
                    <form method="POST" action="{{ route('Jadwal.destroy', $jadwal['id_jadwal']) }}" style="display:inline;">
                        @csrf
                        @method('DELETE')
...
<script>
    document.getElementById("searchInput").addEventListener("keyup", function() {
            ...
            rows.forEach(row => {
                let idJadwal = row.cells[0].textContent.toLowerCase();
                let namaMahasiswa = row.cells[1].textContent.toLowerCase();

                if (idJadwal.includes(filter) || namaMahasiswa.includes(filter)) {
                    row.style.display = "";
                } else {
